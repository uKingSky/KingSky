...
void GUIDEMO_Intro(void) {
  ...
  GUI_DispStringHCenterAt("KingSky", xCenter, y= 15);
  GUI_SetColor(GUI_WHITE);
  GUI_SetFont(&GUI_Font13H_ASCII);
  GUI_DispStringHCenterAt("Chongqing University of"
                          "\nScience and Technology"
                          , xCenter, y += 30);
  GUI_SetColor(GUI_LIGHTRED);
  GUI_SetFont(&GUI_Font16_ASCII);
  GUI_DispStringHCenterAt("Embeded Operating System"
                          , xCenter, y += 40);
  ...
  GUI_SetColor(GUI_WHITE);
  GUI_DispStringHCenterAt("www.cqust.cn", LCD_GET_XSIZE() / 2, LCD_GET_YSIZE() - 40);
  ...
  GUI_DispStringAt("Compiler: ""zhuangtongquan",0, 230);
  GUIDEMO_Delay(5000);
}
