...
/*
        ...
        *********************************************************
*/
#ifdef WIN32   /* Avoid warnings in MS-compiler */  //定义WIN32
...
static tOff CurOff;/static U8  CurData;
...
void LCD_L0_FillRect(int x1,int y1,int x2,int y2) {
   #if LCD_MIRROR_X
    #define X0 (LCD_XSIZE-1-(x1))
    #define X1 (LCD_XSIZE-1-(x0))
  #else
...
int  LCD_L0_Init(void) {
  GUI_DEBUG_LOG("\nLCD_Init()");
   Lcd_Init();
   Lcd_PowerEnable(0, 1);
   Lcd_ClearScr( (0x00<<11) | (0x00<<5) | (0x00) );  //清空
  return 0;    /* Init successfull ! */
}
//使用配置设置再次初始化LCD，不删除显示屏的内容
void  LCD_L0_ReInit(void) {
 Lcd_Init();//再次配置初始化，只是进行初始化，没有删除内容
}
/*
        *********************************************************
        *                                                       *
