...
typedef int   (*CMD)(struct cmd_tbl_s *, int, int, char * const []);
#endif  /*#if KS_CMD_EN > 0*/

#if KS_DEVICE_EN >0
typedef struct {
       void (*device_open)();
       void (*device_read) (uint8 *,uint32);
       void (*device_write)(uint8 *);
       void (*device_ioctl)(uint32,unsigned long);
       void (*device_close)();
       void (*device_state)();
    } file_operations;
	typedef struct _device_node{
		  uint8 *device_name;	//设备名
		  uint8 *state;
		  uint8  device_type; 	 //1-块设备, 2-字符设备;
		  uint8  can_shared;		 //0---不可共享使用, 1-可共享使用
		  uint8  open_count; 	 //对于共享设备，此字段为打开次数计数
		  ks_event *device_sem;
		  file_operations deivce_op; 	 //设备驱动模块提供的设备操作函数表；
		 } device_node;

	
	typedef device_node* KS_DEVICE_HANDLE;
#define BLOCK_DEVICE   1
#define CHAR_DEVICE    2
#define CAN_SHARED     1
#define NOT_CAN_SHARED 0	
...
KS_EXT  volatile  uint32  ks_time;    

#if KS_CMD_EN > 0
KS_EXT cmd_tbl_t __ks_boot_cmd[CMD_MAX_LEN+1];
#endif
