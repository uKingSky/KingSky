...
static char media[2][(long)FS_RR_BLOCKNUM * FS_RR_BLOCKSIZE];
...
static int _FS_RAM_DevRead(FS_u32 Unit, FS_u32 Sector, void *pBuffer) 
{
  ...
  FS__CLIB_memcpy(pBuffer, (media[Unit] + Sector * FS_RR_BLOCKSIZE),
...
static int _FS_RAM_DevWrite(FS_u32 Unit, FS_u32 Sector, void *pBuffer) 
{
  ...
  FS__CLIB_memcpy((media[Unit] + Sector * FS_RR_BLOCKSIZE), pBuffer,
                  (FS_size_t)FS_RR_BLOCKSIZE);
  return 0;
}
