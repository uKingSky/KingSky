...
typedef void (*THREAD_ADDR)(void *)reentrant ;
...
void                 ks_list_insertend( ks_list *list, ks_list_item *newitem) reentrant;

void 				 ks_item_remove( ks_list_item *list_item ) reentrant;//删除链表的节点

void                 ks_system_initialize(void) reentrant;

KS_STACK             * ks_stack_init(THREAD_ADDR thread_addr,void *p_arg,KS_STACK *stk1) reentrant;

uint8 ks_thread_init(ks_thread_block *ptr,KS_STACK *stk,KS_BASE_TYPE prio,const uint8 *const thread_name) reentrant;

ks_thread_block      *ks_thread_allocate(void) reentrant;//申请一块空闲线程

void 				 ks_schedule(void)reentrant;//调度

void				 ks_start(void) reentrant;//系统启动函数

void 				 ks_time_tick(void) reentrant;//节拍服务子程序

void                 ks_int_enter(void) reentrant;//进入中断

void                 ks_int_exit(void) reentrant;
void                 ks_schedule_lock(void) reentrant;  /*调度器上锁*/

void                 ks_schedule_unlock(void) reentrant; /*调度器解锁*/

/*下面是事件控制块的函数*/

/*初始化事件控制块*/
#if KS_EVENT_EN > 0
void                 ks_event_list_init(ks_event *pevent) reentrant; /*使事件等待链表中的线程进入事件的就绪态      */
ks_thread_block     *ks_event_ready(ks_event *pevent,void *msg,uint8 msk) reentrant;/*使线程进入等待某事件发生状态 */
void                 ks_event_wait(ks_event *pevent) reentrant;     /*线程等待超时而将线程置为就绪态               */
void                 ks_event_to(ks_event *pevent) reentrant;
#endif
/*下面是内存管理的函数*/
void                *ks_malloc(uint32 size) reentrant;
void                 ks_free(void *pv) reentrant;
...
KS_HANDLE 			ks_thread_create(THREAD_ADDR thread_addr,
						 			 const uint8 *const thread_name,
						  			 void *p_arg,
						 			 KS_STACK *stk,
						 			 KS_BASE_TYPE prio) reentrant;
void                 ks_thread_idle(void *pram) reentrant;
void				 ks_thread_delay(KS_TIME_TYPE ticks) reentrant;
uint8 				 ks_thread_suspend(KS_HANDLE handle) reentrant;
...
KS_EXT idata  uint8  ks_running;//系统的运行状态

KS_EXT   uint8  lock_nesting;      /*调度器上锁计数器*/

KS_EXT idata ks_thread_block  *current_thread; //volatile
...
KS_EXT  idata  ks_thread_block  *high_thread;

KS_EXT   uint8 int_nesting;

KS_EXT  idata KS_HANDLE h_idle;
